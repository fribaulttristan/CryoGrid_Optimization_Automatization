...
% for CryoGrid for a given sensor
    ...
    S = load(fullfile(result_path_global, 'bayesopt_results.mat'));
    R = S.results;
    ...
    hold on;
    plot(best_result.dates, best_result.T_sim, '--g', 'DisplayName', sprintf('Best simulation (iter %d)', best_result.iteration));
    plot(first_result.dates, first_result.T_sim, ':r', 'DisplayName', sprintf('First simulation (iter %d)', first_result.iteration));
    legend;
    title(sprintf('Simulations comparison — Best (%.4f) vs First (%.4f)', ...
    ...
    fprintf("✅ All best results (score = %.4f) saved to file: %s\n", best_score, output_file);
    ...
    if isprop(results, 'MinObjectiveTrace')
        T.BestSoFar_observed = results.MinObjectiveTrace;
    else
        T.BestSoFar_observed = cummin(results.ObjectiveTrace);
    end
    
    % Best-so-far estimated
    if isprop(results, 'EstimatedObjectiveMinTrace')
        T.BestSoFar_estimated = results.EstimatedObjectiveMinTrace;
    else
        T.BestSoFar_estimated = nan(height(T), 1);
    end
    ...
    % Load merged results including initialization
    ...
    for k = init_idx
        ...
        temp_struct = struct( ...
            ...
            'Objective', -res.score, ...  % negate to match your convention
            'Objective_runtime', NaN, ...
            'BestSoFar_observed', NaN, ...
            'BestSoFar_estimated', NaN ...
        );
    ...
    % Add column Stage (missing in init_rows)
    T_init.Stage = repmat("Initialization", height(T_init), 1);
    ...
    if avg_snow_days_per_year > 15
        ...
        yyaxis left
        plot(R{1}.dates, R{1}.T_obs, 'w-', 'LineWidth', 1.5, 'DisplayName', 'Sensor (reference)');
        ...
        xlabel('Date');
        title('Simulation / Sensor comparison with snowfall');
    ...
    plot(refDates, refTemps, 'w-', 'LineWidth', 1.5, ...
    ...
    bayesopt_file = fullfile(results_global_path, 'bayesopt_results.mat');
