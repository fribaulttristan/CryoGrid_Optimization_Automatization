...
classdef FORCING_slope_seb < matlab.mixin.Copyable
    ...
    methods
        ...
        function forcing = provide_PARA(forcing)
            ...
            forcing.PARA.run_name = [];   %run name
...
%             forcing.TEMP.snowfall = double(forcing.TEMP.Tair <= 2) .* forcing.TEMP.snowfall;
        ...
        function forcing = scale_radiation(PARA, forcing)
            ...
            % save modified forcing data to file [matan]
            output_string=[forcing.PARA.forcing_path extractBefore(forcing.PARA.filename,'.mat') '_' forcing.PARA.run_name '_slope_modified_matan.mat'];
            if isfile(output_string)
                disp('modified slope forcing fil exists')
            else
                disp(['slope modified forcing saved in: ' output_string])
                save(output_string,'forcing')
            end
