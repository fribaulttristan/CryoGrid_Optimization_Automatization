...
% If a higher score is better, the objective is inverted.
    ...
    if n_params < 3
        ...
        y = T.(param_names{2});
        z = T.(param_names{3});
        scores = results.ObjectiveTrace;  % Récupère les scores directement
        
        % Grille régulière (plus dense)
        nGrid = 50;
        ...
        % Seuil bas pour tester
        iso_val = prctile(scores, 20);  
        
        figure;
        p = patch(isosurface(Xq, Yq, Zq, Vq, iso_val));
        isonormals(Xq, Yq, Zq, Vq, p);  % Maintenant, plus d'erreur
        p.FaceColor = 'red';
        p.EdgeColor = 'none';
